...
pipeline/              Go asset generation pipeline
  ...
  cmd/audiogen/        Music + SFX from assets/audio/manifest.yaml (see docs/AUDIO.md)
  cmd/postprocess/     Resize, bg removal, copy to game assets
  internal/audio/      AudioProvider abstraction, manifest, OSS prompts, QA
...
Audio goes through `cmd/audiogen` and `assets/audio/manifest.yaml`, **not**
`cmd/generate -sfx`. The manifest is the source of truth: it carries the prompt,
the seed and the model for every one of the 384 assets, so any of them can be
regenerated byte-for-byte. Full workflow in `docs/AUDIO.md`.

```bash
cd pipeline
go run ./cmd/audiogen -init                       # refresh manifest from skin defs
go run ./cmd/audiogen                             # generate what is missing
go run ./cmd/audiogen -only all -skin <id>        # regenerate one skin
go run ./cmd/audiogen -qa -only all               # check without generating
```

Default provider is `aistack` — local ACE-Step 1.5 (music) and MOSS-SoundEffect
v2.0 (SFX) on the SPARK, reached through `AISTACK_URL`. `AUDIO_PROVIDER=elevenlabs`
still works as a fallback but yields no seed, so its output is not reproducible.

**Licences matter here.** The game ships on Steam, so only models with a
commercial licence may produce assets. Never switch the service to MusicGen,
AudioGen, AudioLDM2, TangoFlux or MMAudio — their weights are non-commercial.

`cmd/generate -sfx` and `-music` are the old direct ElevenLabs path. They still
work, but they write `.mp3` without loudness normalisation or loop crossfade and
record no seed; use them only to reproduce something historical.

### Available skin IDs

`default` · `space_invaders` · `galaga` · `asteroids` · `geometry_wars` · `ikaruga` · `nuclear_throne` · `luftrausers` · `nex_machina` · `tyrian_dos` · `gradius_v` · `rtype` · `river_raid` · `blazing_lazers` · `tempest` · `zaxxon` · `twinbee` · `fantasy_zone` · `abadox` · `solar_striker` · `axelay` · `thunder_force` · `star_fox` · `lords_of_thunder`

A new skin needs a tag translation in `internal/audio/prompts.go` (`musicTags`,
`sfxTags`) — `TestEverySkinHasTagTranslation` fails without it, which is the
point: a skin with no tags would silently get a generic prompt.
