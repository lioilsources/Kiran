...
var Registry = map[string]SkinDef{
	...
	"abadox": {
		...
		// Dark red hulls on red plates: at the default 60 the key took the
		// ship down to 11% opaque and the boss to 10%; from 40 down both hold
		// (22% / 32%) and the set no longer changes (measured 60/40/30/22).
		BgThreshold:       35,
		SpriteSize:        24,
		FrameCount:        6,
		PostProcess:       EffectScanlines,
