...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  MediaKitPlayer(super.id) {
    ...
    _streamSubscriptions = [
      _player.stream.duration.listen((duration) {
        ...
        // Vendored fix: compare the full Duration, not whole seconds — see
        // VENDORED.md. `inSeconds > 0` is false for every sub-second clip.
        if (_setPosition != null && duration > Duration.zero) {
  ...
  Future<SeekResponse> seek(SeekRequest request) async {
    ...
    if (position != null) {
      ...
      // Vendored fix (see VENDORED.md): `inSeconds > 0` skipped the seek for
      // every clip shorter than one second, so a sub-second one-shot could
      // never be rewound after it completed.
